    ...
    constructor()
    {
        ...
        this.multiplierUpgrades = [
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
                ...
                level => new Decimal(1),
                level => Decimal.pow(3, level.pow(game.restackLayer.upgradeTreeNames.resourceMultipliersLevelScaling.apply())).pow(this.getResourceMultiplierBoost())),
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
                level => Utils.createValueDilation(level.mul(3).mul(Decimal.pow(1.1, Decimal.max(0, level.sub(15)))), 0.001).floor().add(10),
                level => new Decimal(1),
                level => Decimal.pow(5, level.pow(game.restackLayer.upgradeTreeNames.resourceMultipliersLevelScaling.apply())).pow(this.getResourceMultiplierBoost())),
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
            ...
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
                ...
                level => new Decimal(1),
                level => Decimal.pow(1000, level.pow(game.restackLayer.upgradeTreeNames.resourceMultipliersLevelScaling.apply())).pow(this.getResourceMultiplierBoost()))
        ];
        ...
        this.powerUpgrades = [
            new MetaDynamicLayerUpgrade("Increase the Resource Power",
                ...
                level => new Decimal(1),
                level => Decimal.pow(1.01, level).pow(this.getResourcePowererBoost()).mul(game.restackLayer.upgradeTreeNames.resourcePowerersStrength.apply()), {
            ...
            new MetaDynamicLayerUpgrade("Increase the Resource Power",
                ...
                level => new Decimal(1),
                level => Decimal.pow(2, level).pow(this.getResourcePowererBoost()).mul(game.restackLayer.upgradeTreeNames.resourcePowerersStrength.apply()), {
                    getEffectDisplay: effectDisplayTemplates.numberStandard(2, "^")
                }),
            new MetaDynamicLayerUpgrade("Increase the Resource Power",
                level => Utils.createValueDilation(level.mul(2e12).mul(Decimal.pow(10, Decimal.max(0, level.sub(1)))), 0.001).floor().add(1e12),
                level => new Decimal(1),
                level => Decimal.pow(8, level).pow(this.getResourcePowererBoost()).mul(game.restackLayer.upgradeTreeNames.resourcePowerersStrength.apply()), {
                    getEffectDisplay: effectDisplayTemplates.numberStandard(2, "^")
                })
        ];
    }
